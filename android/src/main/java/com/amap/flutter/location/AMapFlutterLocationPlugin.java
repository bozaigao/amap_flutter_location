...
public class AMapFlutterLocationPlugin implements FlutterPlugin, MethodCallHandler,
    ...
    public void onMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            ...
            case "addGeoFence":
                ...
                break;
            case "removeGeoFence":
                removeGeoFence((Map) call.arguments);
                break;
            default:
     ...
     * 添加地理围栏
     */
    private void addGeoFence(Map argsMap, Result result) {
        AMapLocationClientImpl locationClientImp = getLocationClientImp(argsMap);
        if (null != locationClientImp) {
            String keyword = (String) argsMap.get("keyword");
            String poiType = (String) argsMap.get("poiType");
            String city = (String) argsMap.get("city");
            Integer size = (Integer) argsMap.get("size");
            String customId = (String) argsMap.get("customId");
            locationClientImp.addGeoFence(result, keyword, poiType, city, size, customId);
        }
    }
